...
public static partial class ZfsPropertyParseRegexes
{
    ...
    /// </summary>
    /// <returns>
    /// Does not parse dataset paths below level 0 (pool roots), snapshots, or inheritance values other than local, default, and none (-).
    /// </returns>
    [GeneratedRegex( "^(?<Name>(?<Dataset>[A-Za-z][A-Za-z0-9_.: -]*?[A-Za-z0-9_.:-]))\t(?<Property>type|sanoid\\.net:[a-z0-9:_.-]+)\t(?<Value>[a-zA-Z0-9@:. _+-]+)\t(?<Source>(?<UndefinedSource>-)|(?<DefaultSource>default)|(?<LocalSource>local))" )]
    ...
    /// </summary>
    /// <returns></returns>
    [GeneratedRegex( "^(?<Name>(?<Dataset>[A-Za-z][A-Za-z0-9_.: -]*?[A-Za-z0-9_.:-](?:/[A-Za-z0-9_.:-][A-Za-z0-9_.: -]*?[A-Za-z0-9_.:-]*?)*))\t(?<Property>type|sanoid\\.net:[a-z0-9:_.-]+)\t(?<Value>[a-zA-Z0-9@:. _+-]+)\t(?<Source>(?<UndefinedSource>-)|(?<DefaultSource>default)|(?<LocalSource>local)|inherited from (?<InheritedSource>[A-Za-z0-9_.: @/-]+))" )]
