...
public readonly partial struct Schema
{
    ...
    public readonly partial struct FormattingEntity
    {
        private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
        {
            ...
            int propertyCount = 0;
            bool foundSnapshotNaming = false;
            foreach (JsonObjectProperty property in this.EnumerateObject())
            {
                if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
                {
                    ...
                    if ((this.HasJsonElementBacking && property.NameEquals(SnapshotNamingUtf8JsonPropertyName.Span)) || (!this.HasJsonElementBacking && property.NameEquals(SnapshotNamingJsonPropertyName)))
                    {
                        foundSnapshotNaming = true;
                    }
                }
            ...
            if (!foundSnapshotNaming)
            {
                if (level >= ValidationLevel.Detailed)
                {
                    result = result.WithResult(isValid: false, $"6.5.3. required - required property \"SnapshotNaming\" not present.");
                }
