...
internal class TemplateConfigurationValidator
{
    ...
    public (bool IsValid, List<TemplateValidationException> ValidationExceptions) ValidateFieldValues( TemplateConfigurationWindow templateWindow )
    {
        ...
        // Naming fields
        NamingComponentSeparator = TemplateWindow.componentSeparatorValidateField.IsValid ? TemplateWindow.componentSeparatorValidateField?.Text?.ToString( ) : null;
        NamingPrefix = TemplateWindow.prefixTextValidateField.IsValid ? TemplateWindow.prefixTextValidateField?.Text?.ToString( ) : null;
        NamingTimestampFormatString = TemplateWindow.timestampFormatTextField?.Text?.ToString( );
        NamingFrequentSuffix = TemplateWindow.frequentSuffixTextValidateField.IsValid ? TemplateWindow.frequentSuffixTextValidateField?.Text?.ToString( ) : null;
        NamingHourlySuffix = TemplateWindow.hourlySuffixTextValidateField.IsValid ? TemplateWindow.hourlySuffixTextValidateField?.Text?.ToString( ) : null;
        NamingDailySuffix = TemplateWindow.dailySuffixTextValidateField.IsValid ? TemplateWindow.dailySuffixTextValidateField?.Text?.ToString( ) : null;
        NamingWeeklySuffix = TemplateWindow.weeklySuffixTextValidateField.IsValid ? TemplateWindow.weeklySuffixTextValidateField?.Text?.ToString( ) : null;
        NamingMonthlySuffix = TemplateWindow.monthlySuffixTextValidateField.IsValid ? TemplateWindow.monthlySuffixTextValidateField?.Text?.ToString( ) : null;
        NamingYearlySuffix = TemplateWindow.yearlySuffixTextValidateField.IsValid ? TemplateWindow.yearlySuffixTextValidateField?.Text?.ToString( ) : null;
        if ( string.IsNullOrWhiteSpace( NamingComponentSeparator ) )
        {
            ...
            TemplateValidationException ex = new( "Name Component Separator", NamingComponentSeparator, TemplateWindow.componentSeparatorValidateField, errorMessage );
            validationExceptions.Add( ex );
            Logger.Warn( ex, errorMessage );
            isValid = false;
        }

        if ( !TemplateWindow.prefixTextValidateField!.IsValid || string.IsNullOrWhiteSpace( NamingPrefix ) )
        {
            string errorMessage = $"Snapshot name prefix value {NamingPrefix} for template {templateName} is invalid";
            TemplateValidationException ex = new( "Name Prefix", NamingPrefix, TemplateWindow.prefixTextValidateField, errorMessage );
            validationExceptions.Add( ex );
            Logger.Warn( ex, errorMessage );
            isValid = false;
        }
        ...
        if ( string.IsNullOrWhiteSpace( NamingTimestampFormatString ) )
        {
            ...
            TemplateValidationException ex = new( "Timestamp Format String", NamingTimestampFormatString, TemplateWindow.timestampFormatTextField, errorMessage );
            validationExceptions.Add( ex );
            Logger.Warn( ex, errorMessage );
            isValid = false;
        }
        else
        {
            ...
            catch ( FormatException ex )
            {
                ...
                TemplateValidationException tve = new( "Timestamp Format String", NamingTimestampFormatString, TemplateWindow.timestampFormatTextField!, errorMessage, ex );
        ...
        if ( !TemplateWindow.frequentSuffixTextValidateField!.IsValid || string.IsNullOrWhiteSpace( NamingFrequentSuffix ) )
        {
            Logger.Warn( "Snapshot template frequent suffix value {0} for template {1} is invalid", NamingFrequentSuffix, templateName );
            isValid = false;
        }

        if ( !TemplateWindow.hourlySuffixTextValidateField!.IsValid || string.IsNullOrWhiteSpace( NamingHourlySuffix ) )
        {
            Logger.Warn( "Snapshot template hourly suffix value {0} for template {1} is invalid", NamingHourlySuffix, templateName );
            isValid = false;
        }

        if ( !TemplateWindow.dailySuffixTextValidateField!.IsValid || string.IsNullOrWhiteSpace( NamingDailySuffix ) )
        {
            Logger.Warn( "Snapshot template daily suffix value {0} for template {1} is invalid", NamingDailySuffix, templateName );
            isValid = false;
        }

        if ( !TemplateWindow.weeklySuffixTextValidateField!.IsValid || string.IsNullOrWhiteSpace( NamingWeeklySuffix ) )
        {
            Logger.Warn( "Snapshot template weekly suffix value {0} for template {1} is invalid", NamingWeeklySuffix, templateName );
            isValid = false;
        }

        if ( !TemplateWindow.monthlySuffixTextValidateField!.IsValid || string.IsNullOrWhiteSpace( NamingMonthlySuffix ) )
        {
            Logger.Warn( "Snapshot template monthly suffix value {0} for template {1} is invalid", NamingMonthlySuffix, templateName );
            isValid = false;
        }

        if ( !TemplateWindow.yearlySuffixTextValidateField!.IsValid || string.IsNullOrWhiteSpace( NamingYearlySuffix ) )
        {
        ...
        ustring monthlyDayString = TemplateWindow.monthlyDayTextValidateField.Text;
        ...
        TimingYearlyDay = TemplateWindow.yearlyDayTextValidateField?.Text?.ToNullableInt32( );
